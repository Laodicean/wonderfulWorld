...
class Server(object):
    ...
    def register(self, path, handler, **kwargs):
        ...
        if isinstance(handler, types.FunctionType):
            ...
            max_nargs = handler.__code__.co_argcount
            ndefaults = handler.__defaults__ and len(handler.__defaults__) or 0
            ...
            class Handler(tornado.web.RequestHandler):
                ...
                def get_fields(self):
                    return dict([(k, v[-1]) for k, v in self.request.args.items()])
            ...
            h = Handler
        elif isinstance(handler, type) and issubclass(handler, (tornado.web.RequestHandler, tornado.websocket.WebSocketHandler)):
            h = handler
        else:
    ...
    def _restart_with_reloader(self):
      while True:
        ...
        if exit_code != 3:
            if exit_code != 0:
                yn = input('Server terminated abnormally, restart? [Yn]')
